...
if __name__ == "__main__":
    ...
    for split in ['train', 'dev', 'test']:
        with open(dir_path/('NQ.' + split + '.idx.json'), 'r',encoding='utf-8') as fin:
            NQ_idx[split] = json.load(fin)
        with open(dir_path/'nq_passages' /  (split + '.json'), 'r',encoding='utf-8') as fin:
            NQ_passages[split] = json.load(fin)


    originaltrain, originaldev = [], []
    with open(dir_path/'NQ-open.dev.jsonl',encoding='utf-8') as fin:
        for k, example in enumerate(fin):
            example = json.loads(example)
            originaldev.append(example)
    
    with open(dir_path/'NQ-open.train.jsonl',encoding='utf-8') as fin:
        for k, example in enumerate(fin):
            example = json.loads(example)
    ...
    originaltrain, originaldev = [], []
    with open(dir_path/'triviaqa-unfiltered'/'unfiltered-web-train.json',encoding='utf-8') as fin:
        originaltrain = json.load(fin)['Data']
    
    with open(dir_path/'triviaqa-unfiltered'/'unfiltered-web-dev.json',encoding='utf-8') as fin:
